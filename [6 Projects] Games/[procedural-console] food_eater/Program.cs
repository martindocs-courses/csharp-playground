/*
    Application should establish the basics of the game, including updating player state, manipulating player movement, and consuming and regenerating a food object.
...
while (!shouldExit)
{
    ...
    {
        Move(nondirectionalKey: true, speedUp: true);
    }
}
...
void Move(bool nondirectionalKey = false, bool speedUp = false)
{
    int lastX = playerX;
    int lastY = playerY;

    bool nonArrowKey = false;
    ...
    switch (Console.ReadKey(true).Key)
    {
        ...
        case ConsoleKey.LeftArrow:
            if (PlayerIncreaseSpeed() && speedUp)
            {
                playerX -= 3;
            }
            else if (PlayerNormalSpeed())
            {
                playerX--;
            }
        ...
        default:
            ...
            {
                Console.Clear();
                Console.WriteLine("Nondirectional input is detected. Program exiting.");
                shouldExit = true;
                nonArrowKey = true;
            };
            break;
...
// Determines if the player has consumed the food that was displayed.
bool ConsumedFood()
{
...
// Method to decrease player movement.
bool PlayerDecreaaseSpeed()
{
    return player.Equals(states[2]);
};

// Method to increase player movement.
bool PlayerIncreaseSpeed()
{
    return player.Equals(states[1]);
};

// Method to normal player movement.
bool PlayerNormalSpeed()
{
    return player.Equals(states[0]);
};